...
#define GENERAL_SETBIT(REG,BIT_VAL, BIT_POS) ((REG) |= ((BIT_VAL) << (BIT_POS)))//ACTIVE LES BITS DANS UN REGISTRE

#define GENERAL_CLEARBIT(REG,BIT_VAL,BIT_POS) ((REG) &=~((BIT_VAL)<<(BIT_POS))) //EFFACE LES BITS DANS UN REGISTRE

#define GENERAL_CFGRBIT(REG,BIT_VAL,BIT_POS,BIT_SIZE) do { GENERAL_SETBIT(REG,BIT_VAL,BIT_POS); GENERAL_CLEARBIT(REG,REVERSE_BIT(BIT_VAL,BIT_SIZE),BIT_POS);} while(0)

#define GENERAL_TOGGLEBIT(REG, BIT_VAL, BIT_POS) ((REG) ^= ((BIT_VAL) << (BIT_POS)))//TOGGLE LES BITS DANS UN REGISTRE


#define REVERSE_BIT(BITVAL,BIT_SIZE)  (((~(BITVAL)) & ((1U<<BIT_SIZE)-1))) //INVERSE LES BITS ET MASQUE AVEC {1U<<BIT_SIZE) - 1}(LE NOMBRE DE BITS À MASQUER) LA RAISON EST D'UTILISER GENERAL_SETBIT ET CLEAR_BIT AVEC 
// LE MÊME BITVAL


#define GENERAL_SETPIN(REG, BIT_VAL, PIN_Pos) (GENERAL_SETBIT(REG, BIT_VAL, 2*PIN_Pos))//ACTIVE LES PINS AU LIEU DES BITS, UTILISE SIMPLEMENT LE PRÉCÉDENT GENERAL_SETBIT

#define GENERAL_CLEARPIN(REG, BIT_VAL, PIN_Pos) (GENERAL_CLEARBIT(REG, BIT_VAL, 2*PIN_Pos))//EFFACE LES PINS AU LIEU DES BITS
